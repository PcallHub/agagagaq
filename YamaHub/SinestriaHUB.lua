...
repeat wait() until game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("LoadingScreen") == nil
end
-- Function 
...
if game.CoreGui:FindFirstChild(getgenv().nill .."," .. getgenv().Version) then
	game.CoreGui:FindFirstChild(getgenv().nill .."," .. getgenv().Version):Destroy()
end
...
nillPaid.Name = getgenv().nill .."," .. getgenv().Version
...
function nill:Window(text)
	...
	Title.Text = text
	Title.TextColor3 = getgenv().Color
	...
	CloseBtn.Text = ""
	CloseBtn.TextColor3 = getgenv().Color
	...
	MinimizeBtn.Text = ""
	MinimizeBtn.TextColor3 = getgenv().Color
	...
	UserTag.TextColor3 = getgenv().Color
	...
	CloseSettingsBtnCircle.BackgroundColor3 = getgenv().Color
	...
	CloseSettingsBtnIcon.BackgroundColor3 = getgenv().Color
	...
	UserPanelUserIcon.MouseButton1Click:Connect(function()
		...
		NotificationHolder.TextColor3 = getgenv().Color
	...
	function nill:Notification(titletext, desctext, btntext)
		...
		Text1.TextColor3 = getgenv().Color
		Text1.TextSize = 20.000
		...
		Text2.Font = Enum.Font.Gotham
		Text2.Text = desctext
		Text2.TextColor3 = getgenv().Color
		Text2.TextSize = 14.000
		...
		AlrightBtn.TextColor3 = getgenv().Color
	...
	function ServerHold:Server(text, LoadImage)
		...
		ServerTitle.TextColor3 = getgenv().Color
		...
		ChannelTitle.Text = ""
		ChannelTitle.TextColor3 = getgenv().Color
		...
		function ChannelHold:Channel(text)
			...
			function ChannelContent:Button(text,callback)
				...
				Button.TextColor3 = getgenv().Color
			...
			function ChannelContent:Toggle(text,default,callback)
				...
				Toggle.Text = ""
				Toggle.TextColor3 = getgenv().Color
				...
				ToggleTitle.TextColor3 = getgenv().Color
				...
				Icon.ImageColor3 = getgenv().Color

				Toggle.MouseButton1Click:Connect(function()
					if toggled == false then
						TweenService:Create(Icon,TweenInfo.new(.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{ImageColor3 = getgenv().Color}):Play()
						TweenService:Create(ToggleFrame,TweenInfo.new(.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{BackgroundColor3 = getgenv().Color}):Play()
			...
			function ChannelContent:Slider(text, min, max, start, callback)
				...
				Slider.Text = ""
				Slider.TextColor3 = getgenv().Color
				...
				SliderTitle.TextColor3 = getgenv().Color
				...
				CurrentValueFrame.BackgroundColor3 = getgenv().Color
			...
			function ChannelContent:Dropdown(text, list, callback)
				...
				DropdownTitle.TextColor3 = getgenv().Color
				...
				CurrentSelectedText.TextColor3 = getgenv().Color
				...
				ArrowImg.ImageColor3 = getgenv().Color
				...
				DropdownFrameBtn.TextColor3 = getgenv().Color
				...
				DropItemHolderLabel.TextColor3 = getgenv().Color
				...
				DropItemHolder.ScrollBarImageColor3 = getgenv().Color
				...
				for i,v in next, list do
					...
					ItemText.TextColor3 = getgenv().Color
			...
			function ChannelContent:Texbox(text, placetext, disapper, callback)
				...
				TextboxTitle.TextColor3 = getgenv().Color
				...
				TextboxFrameOutline.BackgroundColor3 = getgenv().Color
				...
				TextBox.PlaceholderColor3 = getgenv().Color
				TextBox.PlaceholderText = placetext
				TextBox.Text = ""
				TextBox.TextColor3 = getgenv().Color
			...
			function ChannelContent:Label(text)
				...
				Label.Text = ""
				Label.TextColor3 = getgenv().Color
				...
				LabelTitle.TextColor3 = getgenv().Color
...
local Star = nill:Window("Blox Fruits")

local StarServer = Star:Server("Sinestria HUB",8521503225)
...
Main:Toggle("Auto Farm Level",false,function(autofarm)
        getgenv().AutoFarm = autofarm
        end)
spawn(function()
    while wait()do
    if getgenv().AutoFarm then
if game.Players.LocalPlayer.PlayerGui.Main.Quest.Visible == false then CheckQuest() repeat wait() totarget(CFrameQuest) until (game.Players.LocalPlayer.Character.HumanoidRootPart.Position-CFrameQuest.Position).Magnitude <= 4 wait(0.5)local args = {[1] = "StartQuest",[2] = NaemQuest,[3] = LevelQuest} game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack(args)) end end
...
Main:Toggle("Hide Hitbox",true,function(a)
    getgenv().HideHB = a
end)
spawn(function()
    while wait() do
    if getgenv().HideHB then
...
local SelectToolWeapona = Main:Dropdown("SelectWeapon",lol,function(Select)
    getgenv().tool = Select
end)
...
Main:Toggle("Fast Attack", false, function(a)
    getgenv().FastAttack = a 
end)
spawn(function()
...
Main:Toggle("Super Fast Attack", false, function(v) getgenv().super = v
...
Stats:Toggle("Point Defense",nil,function(d)
    getgenv().defense=d end) spawn(function()
        while wait() do
...
Stats:Toggle("Point Melee",nil,function(m)
        getgenv().melee=m end) 
spawn(function()
...
Stats:Toggle("Point Sword",nil,function(s) getgenv().sword=s end) spawn(function()   
    while wait() do if getgenv().sword then
            local args = {
				   [1] = "AddPoint",
				   [2] = "Sword",
				   [3] = 1
...
Stats:Toggle("Point Gun",nil,function(g)
        getgenv().gun=g end)
spawn(function()
        while wait() do
            if getgenv().gun then
...
Stats:Toggle("Point Fruits",nil,function(f)
        getgenv().fruit=f end) 
spawn(function() while wait() do if getgenv().fruit then
